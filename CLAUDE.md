...
- **Entry**: `src/main.tsx` → `src/app/App.tsx`. App.tsx just wires up `react-router-dom` (`BrowserRouter`) with `Navbar` + `Footer` persistent around a single route, `/` (`HomePage`), plus a `*` catch-all that redirects unknown URLs home.
...
- **Portfolio is data-driven**: `src/app/data/tableauProjects.json` is a snapshot of the Tableau Public profile, refreshed on every build by `scripts/fetch-tableau-projects.mjs` (the `prebuild` npm script). `PortfolioShowcase.tsx` renders the first N entries — don't hand-edit the JSON, it gets overwritten.
